...
# from pytorch_object_detection.faster_rcnn.transforms import
import transforms
from network_files.faster_rcnn_framework import FasterRCNN, FastRCNNPredictor
from backbone.resnet50_fpn_model import resnet50_fpn_backbone
# from my_dataset import VOC2012DataSet
from datasets.BottleBodyDataset.dataset import BottleBodyDataSet
...
def main(parser_data):
    ...
    # load train data set
    train_data_set = BottleBodyDataSet(root, data_transform["train"], True)
    ...
    val_data_set = BottleBodyDataSet(root, data_transform["val"], False)
    ...
    model = create_model(num_classes=train_data_set.get_num_classes())
    # print(model)
    ...
    for epoch in range(parser_data.start_epoch, parser_data.epochs):
        ...
        save_files = {
            ...
            'epoch': epoch}
        if epoch % 50 == 0:
            torch.save(save_files, "./save_weights/resNetFpn-model-{}.pth".format(epoch),_use_new_zipfile_serialization=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data-path',
                        default='/media/host/新加卷3/dy/天池缺陷检测比赛/dataset/',
                        help='dataset')
    # 文件保存地址
